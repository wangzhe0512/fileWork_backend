...
- 新增 `PlaceholderType` 枚举：`DATA_CELL / TABLE_CLEAR / LONG_TEXT / BVD`
- 新增 `RegistryEntry` 内部类（`@Data`）：字段含 `placeholderName, displayName, type, dataSource, sheetName, cellAddress`
- 新增 `PLACEHOLDER_REGISTRY` 静态常量（40条映射）：数据表B1~B8、18个财务整表TABLE_CLEAR、行业情况B1~B5、4条BVD
- `ExcelEntry` 新增字段：`placeholderType`（`PlaceholderType`）、`displayName`（`String`）

### T2 · 重构 buildExcelEntries / buildBvdEntries

- `buildExcelEntries()`：遍历注册表→按坐标精确读取，废弃旧 `detectSheetType` 动态扫描；返回顺序：LONG_TEXT → DATA_CELL（值长度降序）→ TABLE_CLEAR
- 新增 `getSheetRowsWithFallback()`：trim+忽略大小写容错匹配Sheet名，带缓存Map
- `buildBvdEntries()`：改为注册表坐标直读，废弃旧企业名称定位行逻辑
- `extractCompanyNameFromEntries()`：优先匹配注册表 `清单模板-数据表-B1`

### T3 · 改造 replaceInRunsNew + 新增辅助方法

- `replaceInRunsNew()` 四路分支：`LONG_TEXT`→整段精确替换；`DATA_CELL`短值(<8字)→词边界正则；`DATA_CELL`长值→直接replace；`BVD`→标uncertain；`TABLE_CLEAR`→跳过
- 新增 `replaceWithWordBoundary(String text, String value, String replacement)`：使用 `(?<![\\u4e00-\\u9fa5A-Za-z0-9])` 中文词边界正则，异常时回退精确替换
- 新增 `clearTableBlock(XWPFDocument doc, List<ExcelEntry> tableClearEntries, List<MatchedPlaceholder> matchedList, List<ExcelEntry> unmatchedEntries)`：识别财务表格（数字列占比≥40%）→整列清空替换为占位符
- 新增 `isFinancialTable(XWPFTable table)`：行≥3、列≥2、数字单元格占比≥40%判定
- `reverse()` 主方法：集成 TABLE_CLEAR 调用（2c步骤）

### T4 · 扩展 ReverseResult + 未匹配检测

- `ReverseResult` 新增字段：`List<RegistryEntry> unmatchedRegistryEntries`
- 新增 `detectUnmatchedRegistry(List<MatchedPlaceholder> matchedList, List<ExcelEntry> tableClearUnmatched)`：对比注册表vs实际matchedList，输出未处理条目
- 删除废弃常量 `BVD_MIN_VALUE_LEN` 和方法 `toColumnLetter()`
